...
$query = "SELECT * FROM reservas WHERE fecha = '$fecha' AND nombre_actividad = '$nombre_actividad' AND ((hora >= '$hora_anterior' AND hora <= '$hora') OR (hora >= '$hora' AND hora <= '$hora_posterior'))";
$result = mysqli_query($con, $query);


if (mysqli_num_rows($result) > 0) {
 echo "<script>alert('La fecha y hora ya están reservadas por otro usuario para la misma actividad.');
 ...
 echo "<script>alert('Reserva realizada correctamente. el pagament es fara allá mateix');
